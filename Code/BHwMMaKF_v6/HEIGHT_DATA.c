...
int16_t planeEquation(uint16_t p1[], uint16_t p2[], uint16_t p3[], uint16_t x, uint16_t y)//x Latitude y longitude
{
  int16_t r[3] = {p2[0]-p1[0],p2[1]-p1[1],p2[2]-p1[2]};
  int16_t s[3] = {p3[0]-p1[0],p3[1]-p1[1],p3[2]-p1[2]};
  int16_t n[3] = {r[1]*s[2] - r[2]*s[1],r[2]*s[0] - r[0]*s[2],r[0]*s[1] - r[1]*s[0]};
...
int16_t searchPrecise(float x, float y, int convert)
{
  ...
  for (i=1; i<MAX; i++)
  {
      ...
      if (diff1 <= minimum1)
      {
          ...
          lonStore[counter] = (int16_t)(pgm_read_word(&height[i].lon));
  ...
  int16_t latStore1 = ((latitude[pos-1] - latStore) > (latStore - latitude[pos+1])) ? latitude[pos+1] : latitude[pos-1];
  int16_t lonStore1 = ((height[loc+1].lon - lonStore[location-1]) > (lonStore[location-1] - height[loc-1].lon)) ? height[loc-1].lon : height[loc+1].lon;
  ...
  int16_t HEIGHT = planeEquation(point1,point2,point3, new_x, new_y);
  return HEIGHT;
}
