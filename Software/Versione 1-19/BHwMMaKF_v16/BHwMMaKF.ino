...
MapMatching mm1;//for baro calibration
#define Pi 3.1415926
...
void MAP_MATCHING()
{
  ...
  if (current_time - MesurementPreviousMillis >= measurement_interval)
  {
    ...
    if (GPS.fix) 
    {
      ...
      //Raw GPS data without any MapMatching improvement
      if (mm.RESET)
      {
      ...
      if (mm1.RESET)
      {
        mm1.begin(raw_lon, raw_lat, true);
        latitude = mm1.yPos;
        longitude = mm1.xPos;
      }else
      {
        mm1.calibrate_GPS(longitude, latitude, true);
        latitude = mm1.yPos;
        longitude = mm1.xPos;
      }
      
      //Get calibration status
      if (mm1.distance() <= mm1.cali_dist || mm.distance() <= mm.cali_dist)
      {
  ...
  if (BUTTON_FLAG_PRESSED[5])
  {
    ...
    mm1.compensation_lon = 0;
    mm1.compensation_lat = 0;
    BUTTON_FLAG_PRESSED[5] = false;
  }
}
