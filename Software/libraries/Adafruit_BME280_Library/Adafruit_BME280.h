...
class Adafruit_BME280 {
    ...
    */
	...
	void setSampling(sensor_mode mode              = MODE_NORMAL,
        ...
        float readAltitude(float seaLevel, float T0, float gradient);
        float seaLevelForAltitude(float altitude, float pressure);
        float readPressureOnHeight(float seaLevel, int altitude, float gradient, float temperatureAtSeaLevel);
        float seaLevelForAltitudeDWD(float altitude, float pressure);
        float readAltitude_DWD(float seaLevel, float gradient);
